...
async function requestHandler(req, res) {
  ...
  if (parts[0] === "api" && parts[1] === "rooms") {
    ...
    if (req.method === "POST" && action === "join") { const nickname = cleanName(body.nickname); if (nickname.length < 2) { sendError(res, 400, "Biệt danh cần từ 2–20 ký tự"); return; } const existingPlayer = getPlayer(room, body.sessionId); if (room.players.length >= room.maxPlayers && !existingPlayer) { sendError(res, 409, `Phòng đã đủ ${room.maxPlayers} người chơi`); return; } const sessionId = existingPlayer ? body.sessionId : makeId(12); let player = existingPlayer; if (!player) { player = { sessionId, nickname, crab: null, ready: false, joinedAt: now(), online: true }; room.players.push(player); } else { player.nickname = nickname; player.online = true; } broadcast(room); await saveRoom(room); sendJson(res, 200, snapshot(room, sessionId)); return; }
