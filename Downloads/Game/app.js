...
const englishReplacements = [
  ["DLICOM SOCIAL MODE", "DLICOM SOCIAL MODE"], ["DLICOM SOCIAL ARENA", "DLICOM SOCIAL ARENA"], ["Kết nối hội bạn.", "Connect your circle."], ["Chọn lane của bạn.", "Pick your lane."], ["Cùng chạy, cùng vui.", "Race together, have fun."], ["Top của room.", "Top of the room."], ["Tạo room Dlicom", "Create a Dlicom room"], ["Vào room", "Join a room"], ["Mở phòng →", "Open room →"], ["Tham gia", "Join"], ["Một mini-game cộng đồng cho phòng Dlicom. Mời hội bạn, chọn cua và xem ai về nhất.", "A social mini-game for your Dlicom room. Invite friends, pick a crab and see who wins."], ["Dán mã room để vào cùng hội bạn.", "Paste the room code to join your circle."], ["DLICOM RACE // LIVE", "DLICOM RACE // LIVE"], ["SOCIAL SPRINT", "SOCIAL SPRINT"], ["DLICOM ROOM · REALTIME", "DLICOM ROOM · REALTIME"], ["Chọn cua cho room", "Choose a crab for the room"], ["DLICOM SPRINT · LIVE", "DLICOM SPRINT · LIVE"], ["Cùng chạy, ", "Race together, "], ["room ơi!", "room, let's go!"], ["DLICOM ROOM · FINISH SYNCED", "DLICOM ROOM · FINISH SYNCED"], ["Top của room: ", "Room top finisher: "], ["race!", "race!"], ["ROOM CHAMPION", "ROOM CHAMPION"], ["Room finish order", "Room finish order"], ["Dlicom Race · quick rules", "Dlicom Race · quick rules"], ["DLICOM RACE là mini-game cộng đồng: chọn cua, bật sẵn sàng và để room quyết định nhà vô địch.", "DLICOM RACE is a social mini-game: pick a crab, get ready and let the room decide its champion."],
...
const dlicomCopyReplacements = [
  ["PHÒNG CHỜ · REALTIME", "DLICOM ROOM · REALTIME"], ["Chọn chiến mã.", "Chọn lane của bạn."], ["Chọn một chú cua", "Chọn cua cho room"], ["ĐƯỜNG ĐUA ĐANG NÓNG · SERVER SYNC", "DLICOM SPRINT · LIVE"], ["Chạy đi, ", "Cùng chạy, "], ["cua ơi!", "room ơi!"], ["CỜ VỀ ĐÍCH ĐÃ HẠ · ĐỒNG BỘ XONG", "DLICOM ROOM · FINISH SYNCED"], ["Một màn ", "Top của room: "], ["kịch tính.", "race!"], ["VUA BÃI BIỂN", "ROOM CHAMPION"], ["Thứ tự về đích", "Room finish order"], ["Luật chơi siêu ngắn", "Dlicom Race · quick rules"], ["Chọn một chú cua, bật sẵn sàng rồi để các chú cua tự chạy. Các làn trống sẽ do BOT điều khiển. Trận đấu kết thúc sau khoảng 20–30 giây — cua về đích đầu tiên là người thắng.", "DLICOM RACE là mini-game cộng đồng: chọn cua, bật sẵn sàng và để room quyết định nhà vô địch."],
];
...
function applyDlicomCopy() {
  ...
  const textNodes = [];
  while (walker.nextNode()) textNodes.push(walker.currentNode);
  textNodes.forEach((node) => { node.nodeValue = dlicomCopyReplacements.reduce((result, [from, to]) => result.split(from).join(to), node.nodeValue); });
}
...
function renderHome() {
  app.innerHTML = `<section class="home-grid"><div class="home-copy"><div class="eyebrow">DLICOM SOCIAL ARENA</div><h1>Kết nối hội bạn.<br />Chọn cua.<br /><em>Đua thôi.</em></h1><p class="lede">Một mini-game cộng đồng cho phòng Dlicom. Mời hội bạn, chọn cua và xem ai về nhất.</p><div class="home-actions"><form class="action-card primary" id="create-form"><h2>Tạo room Dlicom</h2><p>Mở một room riêng và gửi mã cho hội bạn cùng vào.</p><input class="field" id="create-name" maxlength="20" placeholder="Biệt danh của bạn" autocomplete="nickname" /><label class="capacity-field"><span>Số người tối đa</span><select class="field" id="player-count" aria-label="Số người tối đa">${ROOM_SIZES.map((count) => `<option value="${count}" ${count === 6 ? "selected" : ""}>${count} người</option>`).join("")}</select></label><button class="primary-button full" type="submit" style="margin-top: 9px">Mở phòng →</button></form><form class="action-card" id="join-form"><h2>Vào room</h2><p>Dán mã room để vào cùng hội bạn.</p><input class="field" id="join-name" maxlength="20" placeholder="Biệt danh" autocomplete="nickname" /><div class="join-row" style="margin-top: 9px"><input class="field" id="join-code" maxlength="6" placeholder="MÃ ROOM" aria-label="Mã room" /><button class="secondary-button" type="submit">Tham gia</button></div></form></div><p class="microcopy"><strong>6–100 cua</strong> · realtime · vui cùng room</p></div>${renderTrackPreview()}</section>`;
...
function renderTrackPreview() { return `<div class="track-preview"><div class="preview-top"><h2>DLICOM RACE // LIVE</h2><span class="live-tag">● SOCIAL SPRINT</span></div><div class="race-track">${crabs.slice(0, 6).map((item, index) => `<div class="mini-lane"><span class="mini-name">#${String(item.number).padStart(2, "0")} ${item.name}</span><div class="mini-line"><span class="mini-crab" style="--pos:${18 + index * 11}%">${crabImage(item.name)}</span></div><span class="mini-number">0${index + 1}</span></div>`).join("")}</div><span class="sand-dot"></span><div class="preview-caption"><strong>Ai sẽ là room champion?</strong><span>6–100 CUA / ROOM</span></div></div>`; }
